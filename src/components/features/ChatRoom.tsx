...
export const ChatRoom: React.FC<ChatRoomProps> = ({ onViewChange }) => {
  ...
  return (
    <div className={`chat-container ${isLeaving ? 'animate-leave' : ''}`}>
      ...
      <style>{`
        .chat-container { height: 100vh; display: flex; flex-direction: column; background: var(--bg-chat); font-family: 'Inter', sans-serif; position: relative; overflow: hidden; }
        .chat-header { display: flex; align-items: center; padding: 15px 20px; gap: 15px; z-index: 100; color: white; background: rgba(138, 136, 251, 0.4); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255,255,255,0.1); }
        ...
        .room-info-header h2 { margin: 0; font-size: 1.1rem; font-weight: 800; letter-spacing: -0.3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
