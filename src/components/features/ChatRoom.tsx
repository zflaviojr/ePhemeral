...
export const ChatRoom: React.FC<ChatRoomProps> = ({ onViewChange }) => {
  ...
  // Mobile Keyboard fix: Force scroll to top on blur
  const handleBlur = () => {
    setTimeout(() => {
      window.scrollTo(0, 0);
  ...
  return (
    <div className={`chat-container ${isLeaving ? 'animate-leave' : ''}`}>
      ...
      <style>{`
        ...
        .chat-messages-area { flex: 1; padding: 15px; overflow-y: auto; display: flex; flex-direction: column; gap: 15px; min-width: 0; -webkit-overflow-scrolling: touch; }
        ...
        .msg-own .msg-bubble { background: linear-gradient(135deg, var(--primary-color) 0%, var(--primary-dark) 100%); color: white; border-bottom-left-radius: 18px; border-bottom-right-radius: 4px; }
        ...
        .chat-input-pill { flex: 1; border: none; background: #f0f2f5; padding: 16px 22px; border-radius: 25px; outline: none; font-size: 1rem; color: #333 !important; font-weight: 500; -webkit-tap-highlight-color: transparent; }
        
        @media (max-width: 600px) {
          .chat-footer-full { padding: 12px 15px 30px; }
          .chat-input-pill { padding: 12px 18px; font-size: 0.95rem; }
        }
      `}</style>
    </div>
  );
